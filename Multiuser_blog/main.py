...
class BLogs(Handler):
    def get(self):
        list = Check_cookie().cookie_validation(self.request.cookies.get('name_id'))
...
class AddBlogPage(Handler):
    def render_front(self, list = list, title = "", blog = "", error=""):
        self.render("newblog.html", title = title,
                    blog = blog, error = error, user = list[0],
                    link = list[1], login_logout = list[2])
...
class LoginHandler(Handler):
    def get(self):
        list = Check_cookie().cookie_validation(self.request.cookies.get('name_id'))
        self.render('login.html', user = list[0],
                    link = list[1], login_logout = list[2])
    ...
    def post(self):
        ...
        if user and valid_pass(name, password, user.password):
            ...
            self.render("login.html", error = "Invalid Login", user = list[0],
                    link = list[1], login_logout = list[2])
...
class SignupHandler(Handler):
    def get(self):
        list = Check_cookie().cookie_validation(self.request.cookies.get('name_id'))
        self.render('signup.html', user = list[0],
                    link = list[1], login_logout = list[2])
    ...
    def post(self):
        ...
        if flag and User_info.all().filter('name =', user_username).get():
            ...
            self.render("signup.html", invalid_user = error_username,
                        ...
                        invalid_email = error_email, user_error = user_username,
                        passW = user_password, re = user_re_password, mail = user_email,
                        user = list[0], link = list[1], login_logout = list[2])
...
class WelcomeHandler(Handler):
    def get(self):
        list = Check_cookie().cookie_validation(self.request.cookies.get('name_id'))
        if list[0] != "Guest":
            self.render("Welcome.html",name = list[0],
                user = list[0], link = list[1], login_logout = list[2])
        else:
...
class OperationHandler(Handler):
    def get(self,post_key):
        ...
        if blog == None:
            ...
            comments = db.GqlQuery("SELECT * FROM Comments Where blog =:1 ORDER BY commented DESC", post_key)
            time.sleep(0.4)
            self.render("blogpage.html", blog = blog, likes = likes, dislikes = dislikes,
                        comments = comments, user = list[0], link = list[1], login_logout = list[2])
    ...
    def post(self, key):
        ...
        list = Check_cookie().cookie_validation(self.request.cookies.get('name_id'))
        if list[0] != "Guest":
            check_user = db.GqlQuery('SELECT * FROM Blogs WHERE title=:1 and user=:2',
                                    Blogs.get_by_id(int(key)).title, list[0])
            user_id = check_secure_val(self.request.cookies.get('name_id'))
            if like_post_id:

                if check_user.count() == 0:
                    u = db.GqlQuery('SELECT * FROM Likes WHERE blog=:1 and user=:2',
                                     key, user_id)
                    ds = db.GqlQuery('SELECT * FROM Dislikes WHERE blog=:1 and user=:2',
                                     key, user_id)
                    if u.count() == 0:
                ...
                if check_user.count() == 0:
                    like = db.GqlQuery('SELECT * FROM Likes WHERE blog=:1 and user=:2', key, user_id)
                    u  = db.GqlQuery('SELECT * FROM Dislikes WHERE blog=:1 and user=:2', key, user_id)
                    if u.count() == 0:
                ...
                if comment:
                    c = Comments(blog = key, user = User_info.get_by_id(int(
                                 user_id)).name,
                                 comment = comment)
                    c.put()
                else:
                ...
                if User_info.get_by_id(int(user_id)).name == Comments.get_by_id(
                    int(edit_comment)).user:
                    self.redirect("/commentedit/%s" % edit_comment)
                else:
                    error = "you can only edit your own comment"
            elif edit_post:
                if User_info.get_by_id(int(user_id)).name == Blogs.get_by_id(
                    int(edit_post)).user:
            ...
            comments = db.GqlQuery("SELECT * FROM Comments Where blog =:1 ORDER BY commented DESC limit 10", key)
            time.sleep(0.4)
            self.render("blogpage.html", comments = comments, error = error, blog =
                blog, likes = count_likes(key), dislikes = count_dislikes(key),
                user = list[0], link = list[1], login_logout = list[2])
        else:
            self.redirect("/login")
...
class EditCommentHandler(Handler):
    def get(self,comment_key):
        ...
        list = Check_cookie().cookie_validation(self.request.cookies.get('name_id'))
        if list[0] != "Guest":
            self.render("comment.html", value = comment.comment, user = list[0],
                        link = list[1], login_logout = list[2])
        else:
            self.redirect("/login")
    ...
    def post(self,comment_key):
        ...
        list = Check_cookie().cookie_validation(self.request.cookies.get(
            'name_id'))
        if list[0] != "Guest":
            if comment:
                if edit:
                    c = Comments(blog = u.blog, user = u.user, comment =
                        comment, commented = u.commented)
                    u.delete()
                    c.put()
                elif delete:
                    u.delete()
                ...
                self.render("comment.html",
                        error = "Comment Should not be NULL", user = list[0],
                        link = list[1], login_logout = list[2])
        else:
            self.redirect("/login")
...
class EditPostHandler(Handler):
    def get(self, post_id):
        ...
        if list[0] != "Guest" and blog_user != None and list[0] == blog_user.user:
            self.render("editpost.html", title = blog.title,
                value = blog.blog, user = list[0],
                        link = list[1], login_logout = list[2])
        else:
            ...
            if edit:
                ...
                if blog_txt and title_txt:
                    ...
                    self.render("editpost.html", error = error,
                                title = blog.title, value = blog.blog,
